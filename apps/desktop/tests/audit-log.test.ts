...
describe('createAuditLog', () => {
  ...
  describe('path traversal protection', () => {
    test('blocks loading audit log from path outside dirPath using ../', async () => {
      const deps = makeDeps(1000);
      // Create a malicious path that tries to escape the tmpDir
      const maliciousDir = path.join(tmpDir, '..', '..', 'etc');
      
      // The log should be created but load should return empty array
      const log = await createAuditLog(maliciousDir, deps);
      
      // Verify that the log is empty (path traversal blocked)
      expect(log.size()).toBe(0);
      
      // Verify that readFileSync was not called with a path outside tmpDir
      const readCalls = (deps.readFileSync as jest.Mock).mock.calls;
      const auditLogReads = readCalls.filter((call) => 
        String(call[0]).includes('audit-log.json')
      );
      
      // If any reads occurred, they should not have accessed files outside the base
      auditLogReads.forEach((call) => {
        const readPath = String(call[0]);
        const relative = path.relative(path.resolve(maliciousDir), path.resolve(readPath));
        // The path validation should have prevented the read
        expect(relative.startsWith('..') || path.isAbsolute(relative)).toBe(true);
      });
    });

    test('blocks loading audit log from absolute path outside dirPath', async () => {
      const deps = makeDeps(1000);
      // Try to use an absolute path outside the intended directory
      const maliciousDir = '/etc/passwd-dir';
      
      const log = await createAuditLog(maliciousDir, deps);
      
      // The log should be empty due to path traversal protection
      expect(log.size()).toBe(0);
    });

    test('allows loading audit log from valid nested path within dirPath', async () => {
      const deps = makeDeps(1000);
      const validNestedDir = path.join(tmpDir, 'nested', 'audit', 'logs');
      
      const log = await createAuditLog(validNestedDir, deps);
      
      // Should be able to append entries to a valid path
      log.append({
        action: 'test',
        actor: 'dr-test',
        resourceType: 'patient',
    ...
    test('path traversal protection does not affect normal operations', async () => {
      const deps = makeDeps(1000);
      const log = await createAuditLog(tmpDir, deps);
