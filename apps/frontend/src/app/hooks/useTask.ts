...
export const useLoadTasksForPrimaryOrg = () => {
  ...
  useEffect(() => {
    ...
    if (Object.hasOwn(state.taskIdsByOrgId ?? {}, primaryOrgId)) return;
    void loadTasksForPrimaryOrg({ silent: true });
  }, [primaryOrgId]);
};
...
export const useTasksAssignedToUser = (userId?: string): Task[] => {
  ...
  return useMemo(() => {
    ...
    return ids
      .map((id) => tasksById[id])
      .filter((task): task is Task => Boolean(task) && task.assignedTo === userId);
