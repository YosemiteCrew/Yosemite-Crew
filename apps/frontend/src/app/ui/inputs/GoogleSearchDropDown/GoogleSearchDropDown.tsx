    ...
    <div className="w-full relative" ref={dropdownRef}>
      ...
      <div className={`relative`}>
        <input
          ...
          className={`
            h-[46px] w-full border-[1.5px] bg-[var(--field-bg)] px-[14px]
            text-[13.5px] text-[var(--ink-body)] outline-none transition-colors
            placeholder:text-[var(--ink-faint)]
            disabled:cursor-not-allowed disabled:opacity-60
            focus:border-[var(--blue)]! focus:shadow-[0_0_0_3px_var(--glow-b10)]
            ${(() => {
              if (isDropdownOpen) return 'border-[var(--blue)]! rounded-t-[13px]!';
              if (error) return 'border-[var(--danger)]! rounded-[13px]!';
              return 'border-[var(--hairline)]! rounded-[13px]!';
            })()}
          `}
        />
      </div>
      {isDropdownOpen && (
        <div
          className="border-[var(--blue)] max-h-[200px] overflow-y-auto scrollbar-hidden z-99 absolute top-[100%] left-0 rounded-b-[13px] border-l border-r border-b bg-neutral-0 flex flex-col items-center w-full px-[12px] py-[10px]"
