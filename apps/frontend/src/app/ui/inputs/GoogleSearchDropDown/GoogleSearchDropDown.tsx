    ...
    <div className="w-full relative" ref={dropdownRef}>
      ...
      <div className={`relative`}>
        <input
          ...
          className={`
            h-[46px] w-full border-[1.5px] bg-[var(--field-bg)] px-[13px]
            text-[13.5px] text-text-primary outline-none transition-colors
            placeholder:text-input-text-placeholder
            disabled:cursor-not-allowed disabled:opacity-60
            focus:border-input-border-active! focus:shadow-[0_0_0_3px_var(--glow-b10)]
            ${(() => {
              if (isDropdownOpen) return 'border-input-border-active! rounded-t-[13px]!';
              if (error) return 'border-input-border-error! rounded-[13px]!';
              return 'border-input-border-default! rounded-[13px]!';
            })()}
          `}
        />
      </div>
      {isDropdownOpen && (
        <div
          className="border-input-border-active max-h-[200px] overflow-y-auto scrollbar-hidden z-99 absolute top-[100%] left-0 rounded-b-[13px] border-l border-r border-b bg-neutral-0 flex flex-col items-center w-full px-[12px] py-[10px]"
