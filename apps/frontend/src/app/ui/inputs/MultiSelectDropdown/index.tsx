...
const getTriggerClassName = (open: boolean, hasSelection: boolean, error?: string): string => {
  ...
  if (open) {
    borderState = 'border-[var(--blue)]! border-b-0! rounded-t-[13px]! z-20';
  } else if (!hasSelection && error) {
    borderState = 'border-[var(--danger)]! rounded-[13px]!';
  } else {
    borderState = 'border-[var(--hairline)]! rounded-[13px]!';
  }
  ...
  if (open && searchable) {
    return (
      <input
        ...
        className="w-full bg-transparent text-left text-[13.5px] text-[var(--ink-body)] outline-none placeholder:text-[var(--ink-faint)]"
      />
    );
  }
  return (
    <span
      className="min-w-0 flex-1 truncate text-left text-[13.5px] text-[var(--ink-body)]"
