...
// is 0, so an empty field would otherwise read as a real zero. `toDisplayNumber`
// draws that distinction for the whole app now, so "0 u on hand" only shows for
// a genuine zero and cost-only pricing shows when there is no selling price.
// This file used to carry its own copy of that guard.
...
export const buildInventoryPhoneMeta = (item: InventoryItem): InventoryPhoneMeta => {
  ...
  const currentRaw = toDisplayNumber(item.stock?.current);
  ...
  const reorderRaw = toDisplayNumber(item.stock?.reorderLevel);
  ...
  const selling = toDisplayNumber(pricing?.selling);
  const cost = toDisplayNumber(pricing?.purchaseCost);
