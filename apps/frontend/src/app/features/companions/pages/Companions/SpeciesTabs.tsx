...
const SpeciesTabs = ({ counts, activeFilter, onSelect }: SpeciesTabsProps) => (
  <div role="tablist" aria-label="Filter by species" className="flex items-end gap-4">
    {SPECIES_TABS.map((tab) => {
      ...
      return (
        <button
          ...
          className={clsx(
            'inline-flex shrink-0 items-baseline gap-[5px] border-b-2 px-px pb-2 text-[12.5px] transition-colors',
            isActive
        ...
        >
          {tab.label}
          <span className="font-newsreader text-[12.5px] italic text-[var(--ink-faint)]">
            {counts[tab.countKey]}
          </span>
        </button>
      );
