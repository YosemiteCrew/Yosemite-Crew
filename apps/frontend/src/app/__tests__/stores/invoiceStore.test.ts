...
describe('Invoice Store', () => {
  ...
  describe('Bulk Operations', () => {
    it('sets all invoices globally and indexes them correctly', () => {
      useInvoiceStore.getState().setInvoices([mockInvoice1, mockInvoice2, mockInvoice3]);
    ...
    it('skips invoices missing id or organisationId during setInvoices', () => {
      useInvoiceStore.getState().setInvoices([mockInvoice1, mockInvoiceNoId, mockInvoiceNoOrg]);
    ...
    it('retrieves invoices by orgId', () => {
      ...
      const orgAInvoices = useInvoiceStore.getState().getInvoicesByOrgId('org-A');
    ...
    it('skips org-index ids that have no invoice record', () => {
      useInvoiceStore.getState().setInvoices([mockInvoice1]);
      useInvoiceStore.setState((state) => ({
        invoiceIdsByOrgId: {
          ...state.invoiceIdsByOrgId,
          'org-A': ['inv-1', 'inv-missing'],
        },
      }));

      const orgAInvoices = useInvoiceStore.getState().getInvoicesByOrgId('org-A');
      expect(orgAInvoices).toHaveLength(1);
      expect(orgAInvoices).toEqual([mockInvoice1]);
    });

    it('retrieves invoices by status for an org', () => {
      useInvoiceStore.getState().setInvoices([mockInvoice1, mockInvoice2, mockInvoice3]);

      const paidOrgA = useInvoiceStore.getState().getInvoicesByStatus('org-A', 'PAID' as any);
      expect(paidOrgA).toHaveLength(1);
      expect(paidOrgA[0].id).toBe('inv-2');

      const pendingOrgA = useInvoiceStore.getState().getInvoicesByStatus('org-A', 'PENDING' as any);
