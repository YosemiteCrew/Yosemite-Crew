...
describe('Organization Store', () => {
  ...
  describe('Organization CRUD', () => {
    ...
    it('preserves fields beyond the original allow-list, such as check-in settings', () => {
      const orgWithCheckIn: Organisation = {
        ...mockOrg1,
        appointmentCheckInBufferMinutes: 15,
        appointmentCheckInRadiusMeters: 75,
      } as unknown as Organisation;

      const store = useOrgStore.getState();
      store.setOrgs([orgWithCheckIn]);

      expect(useOrgStore.getState().orgsById['org-1']).toMatchObject({
        appointmentCheckInBufferMinutes: 15,
        appointmentCheckInRadiusMeters: 75,
      });
      ...
      store.upsertOrg({
        ...orgWithCheckIn,
        appointmentCheckInBufferMinutes: 30,
    ...
    it('retrieves org by ID', () => {
      ...
      expect(useOrgStore.getState().getOrgById('org-1')).toEqual(
        expect.objectContaining({ name: 'Yosemite Vet' })
      );
