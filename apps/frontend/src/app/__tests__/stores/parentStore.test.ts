...
describe('useParentStore', () => {
  ...
  it('should drop ids that have no stored parent', () => {
    act(() => {
      useParentStore.getState().setParents([mockParent1]);
      useParentStore.setState({ parentIds: ['parent-1', 'missing-parent'] });
    });

    const parents = useParentStore.getState().getAllParents();
    // toHaveLength, not just toEqual: toEqual treats a trailing `undefined`
    // element as absent, so it would pass even without the truthiness guard.
    expect(parents).toHaveLength(1);
    expect(parents).toStrictEqual([mockParent1]);
  });
