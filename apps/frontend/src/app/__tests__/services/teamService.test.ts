...
describe('Team Service', () => {
  ...
  describe('loadTeam', () => {
    ...
    it('fetches data, maps items correctly, and updates store', async () => {
      ...
      expect(axios.getData).toHaveBeenCalledWith('/fhir/v1/user-organization/org/mapping/org-123');
    ...
    it('handles errors during fetch', async () => {
      ...
      expect(mockSetError).toHaveBeenCalledWith('Failed to load team.');
  ...
  describe('sendInvite', () => {
    ...
    it('throws error if no primaryOrgId', async () => {
      ...
      await expect(sendInvite(invitePayload)).rejects.toThrow('No primary organization selected');
      expect(consoleWarnSpy).toHaveBeenCalledWith(
        'No primary organization selected. Cannot send invite.'
      );
    });
    ...
    it('sends post request with correct body', async () => {
      ...
      expect(axios.postData).toHaveBeenCalledWith('/fhir/v1/organization/org-123/invites', {
  ...
  describe('removeMember', () => {
    it('throws error if member ID is missing', async () => {
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith('Failed to delete member:', expect.any(Error));
    });
    ...
    it('deletes member and updates store', async () => {
      ...
      expect(axios.deleteData).toHaveBeenCalledWith('/fhir/v1/user-organization/mem-1');
    ...
    it('logs and throws on API error', async () => {
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith('Failed to delete member:', error);
    });
  });
  ...
  describe('updateMember', () => {
    ...
    it('maps data, puts to API, maps response, and updates store', async () => {
      ...
      // Verify API Call
      expect(axios.putData).toHaveBeenCalledWith('/fhir/v1/user-organization/mem-1', {
        mapped: 'fhir-payload',
      });
