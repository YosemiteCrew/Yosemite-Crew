...
import { getStringified, joinNames } from '@/app/ui/tables/tableUtils';
...
jest.mock('@/app/ui/cards/RoomCard', () => ({
  __esModule: true,
  default: ({ room, handleViewRoom, staffNameById, specialityNameById }: any) => (
    <div data-testid="room-card">
      <span>{room.name}</span>
      <button data-testid={`view-card-${room.name}`} onClick={() => handleViewRoom(room)}>
        View
      </button>
...
describe('RoomTable Component', () => {
  ...
  it('renders RoomCard components with correct lookup maps (Mobile View)', () => {
    ...
    const staffMap = JSON.parse(within(cards[0]).getByTestId('staff-map').textContent ?? '{}');
    expect(staffMap['staff-1']).toBe('Dr. Smith');

    const specMap = JSON.parse(within(cards[0]).getByTestId('spec-map').textContent ?? '{}');
  ...
  it("names its own records in the empty state, not a generic 'No data available'", () => {
    render(<RoomTable filteredList={[]} setActive={mockSetActive} setView={mockSetView} />);

    const messages = screen.getAllByText('No rooms yet');
