...
jest.mock('@/app/lib/specialities', () => ({
  specialties: [{ name: 'Cardiology' }, { name: 'Dermatology' }, { name: 'General Practice' }],
}));
...
describe('SpecialitySearchWeb Component', () => {
  ...
  it("shows 'New speciality' button if query yields no matches", () => {
    render(<SpecialitySearchWeb {...defaultProps} />);

    const input = screen.getByPlaceholderText('Search or create specialty');
    fireEvent.change(input, { target: { value: 'Neurology' } });

    expect(screen.getByText('New speciality “Neurology”')).toBeInTheDocument();
  });

  it("adds a NEW speciality when 'New speciality' is clicked", () => {
    render(<SpecialitySearchWeb {...defaultProps} />);

    const input = screen.getByPlaceholderText('Search or create specialty');
    fireEvent.change(input, { target: { value: 'neurology' } }); // Lowercase input

    fireEvent.click(screen.getByText('New speciality “neurology”'));
  ...
  it('prevents adding empty new speciality', () => {
    ...
    // So "New speciality" button won't appear.
  ...
  it("handles 'multiple=false' for adding new custom speciality", () => {
    ...
    const addBtn = screen.getByText('New speciality “CustomSpec”');
