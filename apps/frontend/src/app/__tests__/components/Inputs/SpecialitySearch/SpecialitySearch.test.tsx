...
jest.mock('@/app/lib/specialities', () => ({
  specialties: [{ name: 'Cardiology' }, { name: 'Dermatology' }, { name: 'Neurology' }],
}));
...
describe('SpecialitySearch Component', () => {
  ...
  it('renders input field correctly', () => {
    render(<SpecialitySearch specialities={[]} setSpecialities={mockSetSpecialities} />);
    expect(screen.getByPlaceholderText('Search or create specialty')).toBeInTheDocument();
  ...
  it('hides already selected specialities from the list', () => {
    ...
    render(<SpecialitySearch specialities={selected} setSpecialities={mockSetSpecialities} />);
    const input = screen.getByPlaceholderText('Search or create specialty');
    fireEvent.focus(input);
  ...
  it('closes dropdown when clicking outside', () => {
    render(
      <div>
        ...
        <SpecialitySearch specialities={[]} setSpecialities={mockSetSpecialities} />
      </div>
    );
  ...
  it("shows 'New speciality' button when no exact match", () => {
    render(<SpecialitySearch specialities={[]} setSpecialities={mockSetSpecialities} />);
    const input = screen.getByPlaceholderText('Search or create specialty');
    fireEvent.change(input, { target: { value: 'Ortho' } });

    expect(screen.getByText('New speciality “Ortho”')).toBeInTheDocument();
  });
  ...
  it('adds new speciality with capitalization', () => {
    ...
    const addBtn = screen.getByText('New speciality “orthodontics”');
  ...
  it('prevents duplicate creation if it already exists in state', () => {
    ...
    // "New speciality 'Cardiology'" button APPEARS.
    ...
    const addBtn = screen.getByText('New speciality “cardiology”');
    fireEvent.click(addBtn);
  ...
  it('does nothing if primaryOrgId is missing', () => {
    ...
    const addBtn = screen.getByText('New speciality “New”');
    fireEvent.click(addBtn);
