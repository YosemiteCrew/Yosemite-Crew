...
import AddAppointment from '@/app/features/companions/components/Sections/AddAppointment';
import {
  LeadOptions,
  SpecialityOptions,
  SupportOptions,
...
jest.mock('@/app/ui/inputs/Dropdown/Dropdown', () => {
  return function MockDropdown({ placeholder, value, onChange, options, error }: any) {
    return (
      <div data-testid={`dropdown-${placeholder.toLowerCase()}`}>
        ...
        {error && <span data-testid={`error-${placeholder.toLowerCase()}`}>{error}</span>}
      </div>
    );
  };
});
...
jest.mock('@/app/ui/inputs/MultiSelectDropdown', () => {
  return function MockMultiSelect({ placeholder, value, onChange, options }: any) {
    return (
...
jest.mock('@/app/ui/inputs/FormDesc/FormDesc', () => {
  return function MockFormDesc({ inlabel, value, onChange }: any) {
    return (
      <div>
        <label>{inlabel}</label>
        <textarea data-testid="desc-concern" value={value} onChange={onChange} />
      </div>
    );
  };
});
