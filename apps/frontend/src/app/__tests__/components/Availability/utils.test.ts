...
jest.mock('@/app/lib/timezone', () => ({
  ...
  utcClockTimeToPreferredTimeZoneClock: jest.fn((value: string) => {
...
describe('Availability Utils', () => {
  ...
  describe('convertAvailability (UI to API)', () => {
    ...
    it('splits overnight intervals into next day segments', () => {
      const input: AvailabilityState = {
        Monday: {
          enabled: true,
          intervals: [{ start: '22:00', end: '05:00' }],
        },
      };
      ...
      const monday = result.availabilities.find((a) => a.dayOfWeek === 'MONDAY');
      const tuesday = result.availabilities.find((a) => a.dayOfWeek === 'TUESDAY');
      expect(monday?.slots).toEqual([{ startTime: '22:00', endTime: '23:59' }]);
      expect(tuesday?.slots).toEqual([{ startTime: '00:00', endTime: '05:00' }]);
    });
  });
  ...
  describe('convertFromGetApi (API to UI)', () => {
    ...
    it('maps cross-midnight UTC slot into two local day slots', () => {
      const apiData: ApiDayAvailability[] = [
        {
          _id: '1',
          organisationId: 'org1',
          dayOfWeek: 'MONDAY',
          slots: [{ startTime: '22:00', endTime: '05:00', isAvailable: true }],
        },
      ];
      ...
      expect(result['Monday'].enabled).toBe(true);
      expect(result['Monday'].intervals).toEqual([{ start: '22:00', end: '23:59' }]);
      expect(result['Tuesday'].enabled).toBe(true);
      expect(result['Tuesday'].intervals).toEqual([{ start: '00:00', end: '05:00' }]);
    });
