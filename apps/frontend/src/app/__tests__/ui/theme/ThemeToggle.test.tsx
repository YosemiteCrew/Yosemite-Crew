...
jest.mock('@/app/ui/theme/useTheme', () => ({
...
describe('ThemeToggle', () => {
  ...
  it('merges style overrides onto the base style', () => {
    render(<ThemeToggle style={{ width: 40, height: 40 }} />);
    expect(screen.getByRole('button')).toHaveStyle({ width: '40px' });
  });

  it('applies the base style when no override is given', () => {
    render(<ThemeToggle />);
    expect(screen.getByRole('button')).toHaveStyle({ width: '38px' });
  });
});
