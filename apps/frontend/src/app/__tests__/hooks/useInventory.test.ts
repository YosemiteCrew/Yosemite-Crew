...
describe('useInventoryModule Hook', () => {
  ...
  const setupStoreMocks = (orgId: string | null, inventoryStateOverrides: any = {}) => {
  ...
  describe('loadInventory', () => {
    it('loads data, maps it, and updates store on success', async () => {
      ...
      (InventoryService.fetchInventoryItems as jest.Mock).mockResolvedValue(mockApiItems);
      (InventoryService.fetchInventoryTurnover as jest.Mock).mockResolvedValue(mockTurnover);
  ...
  describe('createItem', () => {
    it('creates item successfully', async () => {
      ...
      (InventoryService.createInventoryItem as jest.Mock).mockResolvedValue(apiResponse);
  ...
  describe('updateItem', () => {
    it('updates item successfully', async () => {
      ...
      (InventoryService.updateInventoryItem as jest.Mock).mockResolvedValue(apiResponse);
  ...
  describe('hideItem / unhideItem & Merging Logic', () => {
    ...
    it('hideItem: merges existing batches if API returns none', async () => {
      ...
      const expectedMergedCall = expect.objectContaining({
        _id: 'item-1',
        batches: expect.arrayContaining([expect.objectContaining({ _id: 'b1', quantity: 10 })]),
      });
  ...
  describe('updateBatch', () => {
    it('updates only the batches that already have an id', async () => {
      const ORG_ID = 'org-updatebatch';
      setupStoreMocks(ORG_ID);
      const batches = [{ _id: 'batch-1', batch: 'B1' }, { batch: 'B2' }] as any;
      const payload = { some: 'payload' };
      ...
      await act(async () => {
        await result.current.updateBatch('item-1', batches);
      });

      expect(InventoryUtils.buildBatchPayload).toHaveBeenCalledTimes(1);
      expect(InventoryService.updateInventoryBatch).toHaveBeenCalledTimes(1);
      expect(InventoryService.updateInventoryBatch).toHaveBeenCalledWith('batch-1', payload);
      expect(InventoryService.fetchInventoryItems).toHaveBeenCalledWith(ORG_ID);
    });

    it('skips when no batch payload is generated', async () => {
      const ORG_ID = 'org-updatebatch-skip';
      setupStoreMocks(ORG_ID);
      const batches = [{ _id: 'batch-1', batch: 'B1' }] as any;
      (InventoryUtils.buildBatchPayload as jest.Mock).mockReturnValue(null);
      ...
      expect(InventoryService.updateInventoryBatch).not.toHaveBeenCalled();
    });

    it('throws if validation fails', async () => {
      setupStoreMocks('org-updatebatch-fail');
      const { result } = renderHook(() => useInventoryModule('VETERINARY' as BusinessType));

      await expect(result.current.updateBatch('', [])).rejects.toThrow(
        'No inventory item to update.'
      );

      setupStoreMocks(null);
      const { result: res2 } = renderHook(() => useInventoryModule('VETERINARY' as BusinessType));
      await expect(res2.current.updateBatch('id', [])).rejects.toThrow('No organisation selected.');
    });
  });
});
