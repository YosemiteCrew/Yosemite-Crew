 ...
 *
 * An axe pass is CPU-heavy, and this file now runs nineteen of them against a
 * single Next dev server. CI serialises them (`workers: 1`), but a local run is
 * fullyParallel, and adding the twelve public-page tests was enough to push the
 ...
 * pricing, contact, developer and insights surfaces use the full rule set below.
 */
...
test.describe('Public pages — accessibility (WCAG 2.1 AA)', () => {
  ...
  test('insights page has no axe violations', async ({ page }) => {
  ...
  test.describe('marketing pages in dark mode', () => {
    ...
    test('insights page has no dark-theme axe violations', async ({ page }) => {
      await page.goto('/insights');
      await page.waitForLoadState('networkidle').catch(() => {});
