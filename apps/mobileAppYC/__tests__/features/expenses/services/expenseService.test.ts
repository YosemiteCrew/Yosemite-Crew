import expenseApi, {
  ExpenseInputPayload,
} from '../../../../src/features/expenses/services/expenseService';
...
describe('expenseService', () => {
  ...
  describe('fetchSummary', () => {
    it('fetches summary with fallbacks', async () => {
      ...
      const result = await expenseApi.fetchSummary({
        companionId: 'c1',
        accessToken: 't',
        currencyCode: 'EUR',
      });
  ...
  describe('fetchExpenseById', () => {
    it('fetches single expense', async () => {
      ...
      const result = await expenseApi.fetchExpenseById({
        expenseId: 'single-1',
        accessToken: 't',
      });
  ...
  describe('createExternal', () => {
    ...
    it('uploads files and creates expense', async () => {
      ...
      const input = {
        ...baseInput,
        attachments: [fileToUpload, fileExisting] as any,
      };

      (documentApi.uploadAttachment as jest.Mock).mockResolvedValue({
        key: 'uploaded-key.jpg',
      });
    ...
    it('throws error if file has no URI or Key', async () => {
      ...
      await expect(
        expenseApi.createExternal({input: badInput, accessToken: 't'}),
  ...
  describe('updateExternal', () => {
    it('patches expense successfully', async () => {
      ...
      await expenseApi.updateExternal({
        expenseId: 'exp-1',
        input,
        accessToken: 't',
      });
  ...
  describe('deleteExpense', () => {
    it('deletes successfully', async () => {
      ...
      const res = await expenseApi.deleteExpense({
        expenseId: 'del-1',
        accessToken: 't',
      });
  ...
  describe('fetchInvoice', () => {
    beforeEach(() => {
      (mapInvoiceFromResponse as jest.Mock).mockReturnValue({
        invoice: {id: 'mapped-inv'},
        paymentIntent: null,
      });
    });
    ...
    it('handles standard object response', async () => {
      ...
      const res = await expenseApi.fetchInvoice({
        invoiceId: 'inv-1',
        accessToken: 't',
      });

      expect(apiClient.get).toHaveBeenCalledWith(
        '/v1/finance/mobile/inv-1',
        expect.objectContaining({headers: {Authorization: 'Bearer t'}}),
      );
      expect(res.invoice).toEqual(expect.objectContaining({id: 'mapped-inv'}));
    ...
    it('handles direct array response', async () => {
      ...
      await expenseApi.fetchInvoice({invoiceId: '1', accessToken: 't'});
      expect(mapInvoiceFromResponse).toHaveBeenCalledWith({
        id: 'inv-direct-arr',
      });
    });
  ...
  describe('fetchPaymentIntent & fetchPaymentIntentByInvoice', () => {
    it('fetchPaymentIntent maps response correctly', async () => {
      ...
      expect(apiClient.get).toHaveBeenCalledWith(
        '/v1/finance/mobile/payment-intent/pi_1',
        expect.objectContaining({headers: {Authorization: 'Bearer t'}}),
      );
    ...
    it('fetchPaymentIntent resolves invoice lookup through a finance payment session', async () => {
      (mapInvoiceFromResponse as jest.Mock).mockReturnValueOnce({
        invoice: {id: 'inv-from-intent'},
        paymentIntent: null,
      });
      (apiClient.get as jest.Mock).mockResolvedValue({
        data: {data: {id: 'inv-from-intent', status: 'AWAITING_PAYMENT'}},
      });
      (apiClient.post as jest.Mock).mockResolvedValue({
        data: {
          data: {
            providerPaymentIntentId: 'pi_session',
            clientSecret: 'sec_session',
            amount: 500,
            currency: 'EUR',
      ...
      expect(apiClient.post).toHaveBeenCalledWith(
        '/v1/finance/mobile/invoices/inv-from-intent/payments/sessions',
        {provider: 'STRIPE'},
        expect.objectContaining({headers: {Authorization: 'Bearer t'}}),
      );
      expect(res.paymentIntentId).toBe('pi_session');
      expect(res.clientSecret).toBe('sec_session');
      expect(res.amount).toBe(500);
      expect(res.paymentLinkUrl).toBe('https://checkout.example/session');
    });

    it('fetchPaymentIntentByInvoice creates a finance payment session', async () => {
      (apiClient.post as jest.Mock).mockResolvedValue({
        data: {
          data: {
            providerPaymentIntentId: 'pi_inv_1',
            clientSecret: 'sec',
            amount: 200,
            checkoutUrl: 'https://checkout.example/inv-1',
          },
        },
      });
      ...
      expect(apiClient.post).toHaveBeenCalledWith(
        '/v1/finance/mobile/invoices/inv-1/payments/sessions',
        {provider: 'STRIPE'},
        expect.objectContaining({headers: {Authorization: 'Bearer t'}}),
      );
      expect(res.paymentIntentId).toBe('pi_inv_1');
      expect(res.amount).toBe(200);
      expect(res.paymentLinkUrl).toBe('https://checkout.example/inv-1');
    });
  });
