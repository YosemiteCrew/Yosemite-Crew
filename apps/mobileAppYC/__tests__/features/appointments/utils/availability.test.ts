...
describe('availability utils', () => {
  ...
  beforeAll(() => {
    ...
    (MockDateTimeFormat as any).supportedLocalesOf =
  ...
  describe('getFirstAvailableDate', () => {
    it('returns fallback or today if availability is missing', () => {
      expect(availabilityUtils.getFirstAvailableDate(null, mockToday)).toBe(
        mockToday,
      );
      expect(
        availabilityUtils.getFirstAvailableDate(
          undefined,
          mockToday,
          'fallback',
        ),
    ...
    it('returns the first date that has available slots on or after today', () => {
      ...
      const result = availabilityUtils.getFirstAvailableDate(
        availability,
        mockToday,
      );
    ...
    it('returns fallback if no future slots match', () => {
      ...
      expect(
        availabilityUtils.getFirstAvailableDate(availability, mockToday),
      ).toBe(mockToday);
    });
  });
  ...
  describe('getSlotsForDate', () => {
    it('returns empty array if date is in the past', () => {
      expect(
        availabilityUtils.getSlotsForDate({}, '2020-01-01', mockToday),
      ).toEqual([]);
    });

    it('returns empty array if availability is missing', () => {
      expect(
        availabilityUtils.getSlotsForDate(null, '2023-12-01', mockToday),
      ).toEqual([]);
    });
    ...
    it('returns empty array if no slots found for date', () => {
      ...
      expect(
        availabilityUtils.getSlotsForDate(avail, '2023-12-01', mockToday),
      ).toEqual([]);
    });
    ...
    it('filters out unavailable slots', () => {
      ...
      const result = availabilityUtils.getSlotsForDate(
        avail,
        '2023-12-01',
        mockToday,
      );
    ...
    it('formats slots using raw string normalization (Priority 3 - ISO Date)', () => {
      ...
      const avail = {
        slotsByDate: {
          '2023-12-01': [
            {startTime: '2023-12-01T14:30:00Z', isAvailable: true},
          ],
        },
      } as any;
      const result = availabilityUtils.getSlotsForDate(
  ...
  describe('getFutureAvailabilityMarkers', () => {
    it('marks only the days that hold a bookable slot', () => {
      const avail = {
        slotsByDate: {
      ...
      const markers = availabilityUtils.getFutureAvailabilityMarkers(
        avail,
        mockToday,
      );

      // Exactly the two future days with slots - not a blanket window.
    ...
    it('marks nothing when there is no availability at all', () => {
      // This asserted 30 markers, which is precisely the defect: a speciality
      // with no practitioner assigned returns no slots, and the calendar wore
      // a month of dots that every one of them then contradicted.
      const markers = availabilityUtils.getFutureAvailabilityMarkers(
        null,
        mockToday,
      );
    ...
    it('marks nothing when every slot in range is unavailable', () => {
      const avail = {
        slotsByDate: {'2023-10-02': [{isAvailable: false}]},
      } as any;
      const markers = availabilityUtils.getFutureAvailabilityMarkers(
        avail,
        mockToday,
  ...
  describe('parseSlotLabel', () => {
    it('returns nulls for missing value', () => {
      expect(availabilityUtils.parseSlotLabel(null)).toEqual({
        startTime: null,
        endTime: null,
      });
      expect(availabilityUtils.parseSlotLabel(undefined)).toEqual({
        startTime: null,
        endTime: null,
      });
    });
  ...
  describe('findSlotByLabel', () => {
    it('returns null if availability or label is missing', () => {
      expect(
        availabilityUtils.findSlotByLabel(null, mockToday, 'label'),
      ).toBeNull();
      expect(
        availabilityUtils.findSlotByLabel({} as any, mockToday, null),
      ).toBeNull();
    });

    it('returns null if no slots for date', () => {
      expect(
        availabilityUtils.findSlotByLabel(
          {slotsByDate: {}} as any,
          mockToday,
          'label',
        ),
    ...
    it('returns null if no match found', () => {
      ...
      expect(
        availabilityUtils.findSlotByLabel(avail, mockToday, 'Different Time'),
      ).toBeNull();
    });
  });
});
