...
const OPTION_SEEDS: Array<[string, string, ImageKey?]> = [
  ...
  [
    'rest-uncomfortable',
    'Looks uncomfortable, may whimper, worried facial expression',
  ],
  [
    'rest-guarding',
    'Unsettled, crying/groaning, guarding wound, reluctant to move',
  ],
  [
    'rest-severe',
    'Constant crying, screaming, unresponsive, difficult to distract',
  ],
  ...
  [
    'palpation-intense',
    'Cries even at non-painful touch, may react aggressively',
  ],
  ...
  [
    'orbital-not-present',
    'Orbital tightening not present',
    'equineOrbitalNotPresent',
  ],
  [
    'orbital-moderate',
    'Orbital tightening moderately present',
    'equineOrbitalModerate',
  ],
  [
    'orbital-obvious',
    'Orbital tightening obviously present',
    'equineOrbitalObvious',
  ],
...
const OPTION_LIBRARY = OPTION_SEEDS.reduce<Record<string, OptionSpec>>(
  (acc, [id, title, imageKey]) => {
...
const STEP_SEEDS: Array<[string, string, string[]]> = [
  [
    'fgs-ear-position',
    'Ear Position',
    ['ears-forward', 'ears-apart', 'ears-outwards'],
  ],
  [
    'fgs-orbital-tightening',
    'Orbital Tightening',
    ['eyes-opened', 'eyes-partially-closed', 'eyes-squinted'],
  ],
  [
    'fgs-muzzle-tension',
    'Muzzle Tension',
    ['muzzle-relaxed', 'muzzle-mild', 'muzzle-tense'],
  ],
  [
    'fgs-whisker-change',
    'Whisker Change',
    ['whisker-loose', 'whisker-slightly-curved', 'whisker-forward'],
  ],
  [
    'fgs-head-position',
    'Head Position',
    ['head-above-shoulder', 'head-aligned', 'head-below'],
  ],
  [
    'caps-behaviour-rest',
    'Behaviour at Rest',
  ...
  [
    'egs-ear-position',
    'Stiffly backwards ear',
    ['ear-not-present', 'ear-moderate', 'ear-obvious'],
  ],
  [
    'egs-orbital-tightening',
    'Orbital tightening',
    ['orbital-not-present', 'orbital-moderate', 'orbital-obvious'],
  ],
  [
    'egs-eye-tension',
    'Tension above the eye area',
    ['eye-not-present', 'eye-moderate', 'eye-obvious'],
  ],
  [
    'egs-chewing-muscle',
    'Prominently strained chewing muscles',
    ['chewing-not-present', 'chewing-moderate', 'chewing-obvious'],
  ],
  [
    'egs-mouth-strain',
    'Mouth strained and pronounced chin',
    ['mouth-not-present', 'mouth-moderate', 'mouth-obvious'],
  ],
  [
    'egs-nostril-profile',
    'Strained nostrils and flattening of the profile',
    ['nostril-not-present', 'nostril-moderate', 'nostril-obvious'],
  ],
];

const STEP_LIBRARY = STEP_SEEDS.reduce<Record<string, StepSpec>>(
...
const TOOL_SEEDS: Array<[ToolId, ToolSpec]> = [
  ...
  [
    'canine-acute-pain-scale',
    {
      ...
      subtitle:
        "Observe your dog's behaviour and relations. Check all that apply for each pain score level.",
  ...
  [
    'equine-grimace-scale',
    {
      ...
      subtitle:
        "Pay attention to your horse's behavior and interactions. For each pain score level, check all that apply.",
      footer:
...
const createDefinition = (
  ...
  steps: spec.stepIds.map(stepId => {
    ...
    return {
      ...
      // Every category contributes to the composite pain score these
      // clinical scales produce, so none of them is optional. Left
      // unset, ObservationalToolScreen's isStepCompleted() treated an
      // unanswered step as already complete, so the whole instrument
      // (FGS/CAPS/EGS) could be submitted with zero answers selected.
      required: true,
...
export const observationalToolDefinitions: ObservationalToolDefinitionMap =
  TOOL_SEEDS.reduce((acc, [toolId, spec]) => {
