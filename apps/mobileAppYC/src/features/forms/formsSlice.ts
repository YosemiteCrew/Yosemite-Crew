...
const mergeEntries = (
  ...
  const upsert = (entry: AppointmentFormEntry) => {
    const submissionKey = entry.submission?._id
    ...
    if (!submissionKey) {
      const hasSubmittedForForm = Array.from(byKey.values()).some(
        existingEntry =>
          existingEntry.form._id === entry.form._id && existingEntry.submission,
      );
...
const fetchAppointmentFormsData = async ({
  ...
  try {
    const appointmentForms = await formApi.fetchFormsForAppointment({
      appointmentId,
      serviceId,
      species,
      accessToken,
...
>(
  'forms/fetchAppointmentForms',
  async ({appointmentId, serviceId, species}, {rejectWithValue, getState}) => {
    try {
      ...
      const appointmentEntries = await fetchAppointmentFormsData({
        appointmentId,
        serviceId,
        species,
        accessToken,
      ...
      const cacheObject = Array.from(cache.values()).reduce<
        Record<string, Form>
      >((acc, form) => {
  ...
  async (payload, {getState, rejectWithValue}) => {
    try {
      ...
      const submission: FormSubmission = {
        ...
        formVersion:
          payload.formVersion ?? resolveFormVersion(payload.form) ?? 1,
  ...
  extraReducers: builder => {
    builder
      ...
      .addCase(submitAppointmentForm.fulfilled, (state, action) => {
        ...
        state.formsCache = {
          ...state.formsCache,
          [form._id]: normalizeFormForState(form),
        };
      })
      ...
      .addCase(startFormSigning.fulfilled, (state, action) => {
        const {appointmentId, submissionId, signingUrl, documentId} =
          action.payload;
...
export const selectFormsForAppointment = (
  state: RootState,
  appointmentId: string,
