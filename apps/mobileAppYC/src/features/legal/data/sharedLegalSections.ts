...
export const createMobileRecipientsList = (): OrderedListBlock =>
  ol(
    oli(
      '•',
...
export const createDataSubjectRightsSection = (): LegalSection => ({
  ...
  blocks: [
    ...
    p(
      b('Art. 15 GDPR – Right of access by the data subject:\n'),
      seg(
...
export const createCompanyIntro = (
  ...additionalSegments: TextSegment[]
): ParagraphBlock =>
  p(
    seg(
      `${COMPANY_NAME}, ${COMPANY_FULL_ADDRESS} ("DuneXploration" or "we/us/our")`,
    ),
    ...additionalSegments,
  );
...
export const createBookingRecipientsList = (
...
export const createNumberedParagraph = (
  number: string,
  ...segments: TextSegment[]
): ParagraphBlock => p(b(number), ...segments);
...
export const createDefinition = (
  ...
  term: string,
  definition: string,
): ParagraphBlock => p(b(number), b(` ${term} `), seg(definition));

// Factory for creating subsection header paragraphs
export const createSubsectionHeader = (
  number: string,
  title: string,
): ParagraphBlock => p(b(number), b(` ${title}`));

// Factory for creating subsection with bold title
export const createSubsection = (
  number: string,
  title: string,
): ParagraphBlock => p(b(number), seg(` ${title}`));
...
export const createBoldHeader = (text: string): ParagraphBlock => p(b(text));

// Factory for creating multi-segment paragraph
export const createMultiSegmentParagraph = (
  ...segments: TextSegment[]
): ParagraphBlock => p(...segments);
