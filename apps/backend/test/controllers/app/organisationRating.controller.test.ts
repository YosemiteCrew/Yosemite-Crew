...
describe("OrganisationRatingController", () => {
  ...
  describe("rateOrganisation", () => {
    it("should 400 if rating is missing in body", async () => {
      ...
      await OrganisationRatingController.rateOrganisation(
        req as any,
        res as Response,
      );
    ...
    it("should success (200) with userId from request object (middleware)", async () => {
      ...
      (mockedRatingService.rateOrganisation as any).mockResolvedValue(
        undefined,
      );
    ...
    it("should handle errors (500)", async () => {
      (req as any).userId = "u1";
      mockGenericError(
        mockedAuthService.getByProviderUserId as unknown as jest.Mock,
      );
  ...
  describe("isUserRatedOrganisation", () => {
    it("should 400 if parentId not found", async () => {
      ...
      await OrganisationRatingController.isUserRatedOrganisation(
        req as any,
        res as Response,
      );
    ...
    it("should success (200) returning true", async () => {
      ...
      (mockedRatingService.isUserRatedOrganisation as any).mockResolvedValue(
        true,
      );
      ...
      expect(mockedRatingService.isUserRatedOrganisation).toHaveBeenCalledWith(
        "o1",
        "p1",
      );
      expect(statusMock).toHaveBeenCalledWith(200);
    ...
    it("should handle errors (500)", async () => {
      ...
      mockGenericError(
        mockedRatingService.isUserRatedOrganisation as unknown as jest.Mock,
      );
