...
- [x] The `ci.yaml` pipeline gates the full current `jest-axe` suite on every PR touching `apps/frontend/src/**` (the jest-axe specs are ordinary `*.test.tsx` files run in the sharded frontend test stage; the standalone `frontend-a11y.yml` workflow was folded in)
...
2. The `ci.yaml` pipeline (which runs the jest-axe suite in its test stage) must pass green
